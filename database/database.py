...
db_account = TinyDB('db_account.json')
db_message = TinyDB("db_message.json")
user = Query()
...
def register(name, mdp):
    ...
    if not len(result) > 0:
        if name[0].isalpha():
            db_account.insert({"name": name, "mdp": str(mdp), "message": [], "pdp_filename": "default"})
...
def login(name,mdp):
    ...
    if len(result) == 0:
        ...
        if result[0]["mdp"] == mdp:
            ...
            return 1,f"Je crains que vous vous êtes trompés de mot de passe{name.capitalize()}"


def send(name,message):
    now = datetime.now()
    db_account.update(add("message", [{"message":message,"date":now.strftime("%d/%m/%Y %H:%M:%S")}]), user.name == name)
    db_message.insert({"message":message,"date":now.strftime("%d/%m/%Y %H:%M:%S"),"name":name})


def message_from(name):
    return db_account.search(user.name == name)[0]["message"]
